...
public class JcReportParser extends AbstractWarningsParser {
    ...
    // private transient Report report;
     private List<FileAnnotation> warnings;
    ...
    /**
     * Sets the warnings to the specified value.
     * @param warnings the value to set
     */
    public void setWarnings(final List<FileAnnotation> warnings) {
     ...
     *  @author Johann Vierthaler, johann.vierthaler@web.de
     *  This overwritten method passes the reader to createReport() and
     *  starts adding all the warnings to the Collection that will be returned
     ...
     *  @exception IOException -> thrown by createReport()
     *  @exception ParsingCanceledException -> thrown by createReport()
     */
    @Override
    public Collection<FileAnnotation> parse(final Reader reader) throws IOException, ParsingCanceledException {
        ...
        warnings = new ArrayList<FileAnnotation>();
        ...
        for (int i = 0; i < report.getFiles().size(); i++) {
            ...
            for (int j = 0; j < file.getItems().size(); j++) {
               final Item item = file.getItems().get(j);
                final Warning warning = createWarning(file.getName(),
                getLineNumber(item.getLine()),
                item.getFindingtype(),
                item.getMessage(),
                getPriority(item.getSeverity()));
     ...
     * @return warnings -> unmodifiable.
     */
    public List<FileAnnotation> getWarnings() {
